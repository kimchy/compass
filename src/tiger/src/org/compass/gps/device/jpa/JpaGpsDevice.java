 ...
 * <p>
 * The jpa device provides support for using jpa to index a database.
 * The path can be viewed as: Database <-> EntityManager(JPA) <-> Objects <-> Compass::Gps
 * <-> Compass::Core (Search Engine).
 * What it means is that for every object that has both jpa and compass
 ...
 * <p>
 * Indexing uses {@link JpaEntitiesLocator} to locate all the entities that can be indexed
 * (i.e. entities that have both Compass and JPA mappings). Most of the time the
 * {@link org.compass.gps.device.jpa.entities.DefaultJpaEntitiesLocator} is enough,
 * but special JPA implementation one can be provided. If none is provided, the device will
 ...
 * framework or by themself wrap the actual <code>EntityManagerFactory</code> implementation.
 * <p>
 * For advance usage, the device allows for {@link EntityManagerWrapper} to be set, allowing to control the
 * creation of <code>EntityManager</code>s, and transactions. The {@link DefaultEntityManagerWrapper} should
