 ...
 * <code>Compass</code>. <p/> Will perform the search operation on the
 * <code>Compass</code> instance using the query supplied by the
 * {@link CompassSearchCommand}. <p/> If you wish to enable the pagination
 ...
 * {@link CompassSearchCommand}. <p/> The controller has two views to be set,
 ...
 * operation (they can be the same page). <p/> The results of the search
 * operation will be saved under the
 * <code>searchResultsName<code>, which defaults to "searchResults".
 * <p/>
 * Note that the <code>transactionManager<code> which is the <code>PlatformTransactionManager<code>
 * used in the application can be set as well since the search operation is transactional and if it
 * uses the {@link org.compass.spring.transaction.SpringSyncTransaction}, a spring based transaction must be in process.
 *
 * @author kimchy
 */
